         ...
         void *value)
 {
     ...
     if (*key < node->splitchar) {
         ...
         if (len > 1) {
             ...
             assert(node->value == NULL && "Próba wstawienia duplikatu do tst.");
                 node->value = value;
         }
     } else {
         node->high = TSTree_insert_base(
             root, node->high, key, len, valueP
     }
 ...
 void *TSTree_search_prefix(TSTree * root, const char *key, size_t len)
 {
     ...
     // Poruszamy się aż do znalezienia pierwszej wartości w łańcuchu,
     // która następnie będzie pierwszym węzłem o danym prefiksie.
